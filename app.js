(function() {
  // iOS Safari: prevent page jump when keyboard opens
  // Uses visualViewport API which is reliable on iOS 13+
  var savedY = 0;
  var isEditing = false;
  ...
  function onFocusOut() {
    if (!isEditing) return;
    isEditing = false;
    // Wait for keyboard to fully close, then restore position
    setTimeout(function() {
...
const EINRICHTUNG_KATEGORIEN = [
  { id: "krankenhaus", label: "\u{1F3E5} Krankenhaus", farbe: "#D46B6B" },
  { id: "altenheim", label: "\u{1F3E0} Altenheim", farbe: "#C5A55A" },
  { id: "pflegeheim", label: "\u{1F6CF} Pflegeheim", farbe: "#C5A55A" },
  { id: "standesamt", label: "\u{1F3DB} Standesamt", farbe: "#5B9EC9" },
  { id: "gerichtsmedizin", label: "\u{1F52C} Gerichtsmedizin", farbe: "#9B59B6" },
  { id: "friedhof", label: "\u26EA Friedhof", farbe: "#4FA87A" },
  { id: "kirche", label: "\u271D Kirche / Moschee", farbe: "#4FA87A" },
  { id: "krematorium", label: "\u{1F525} Krematorium", farbe: "#E67E22" },
  { id: "arztpraxis", label: "\u2695 Arztpraxis", farbe: "#D46B6B" },
  { id: "polizei", label: "\u{1F694} Polizei", farbe: "#2C3E50" },
  { id: "notar", label: "\u2696 Notar", farbe: "#8E44AD" },
  { id: "sonstige", label: "\u{1F4CB} Sonstige", farbe: "#888" }
];
const TARIF_FELDER = {
  krankenhaus: [{ key: "liegegebuehr", label: "Liegegeb\xFChr pro Tag", einheit: "\u20AC/Tag" }],
  altenheim: [{ key: "liegegebuehr", label: "Liegegeb\xFChr pro Tag", einheit: "\u20AC/Tag" }],
  pflegeheim: [{ key: "liegegebuehr", label: "Liegegeb\xFChr pro Tag", einheit: "\u20AC/Tag" }],
  standesamt: [{ key: "sterbeurkunde", label: "Sterbeurkunde (pro St\xFCck)", einheit: "\u20AC" }],
  gerichtsmedizin: [
    { key: "obduktion", label: "Obduktionsgeb\xFChr", einheit: "\u20AC" },
    { key: "liegegebuehr", label: "Liegegeb\xFChr pro Tag", einheit: "\u20AC/Tag" }
  ],
  friedhof: [
    { key: "grabgebuehr", label: "Grabgeb\xFChr (Erstbelegung)", einheit: "\u20AC" },
    { key: "nutzungsrecht", label: "Nutzungsrecht (Jahre)", einheit: "Jahre" },
    { key: "bestattungsgebuehr", label: "Bestattungsgeb\xFChr", einheit: "\u20AC" }
  ],
  krematorium: [
    { key: "einaescherung", label: "Ein\xE4scherungsgeb\xFChr", einheit: "\u20AC" },
    { key: "wartezeit", label: "Wartezeit (Werktage)", einheit: "Tage" }
  ],
  kirche: [{ key: "kirchengebuehr", label: "Kirchliche Geb\xFChr", einheit: "\u20AC" }],
  arztpraxis: [{ key: "totenschein", label: "Totenschein-Geb\xFChr", einheit: "\u20AC" }],
  polizei: [],
  notar: [{ key: "beglaubigung", label: "Beglaubigungsgeb\xFChr", einheit: "\u20AC" }],
  sonstige: [{ key: "gebuehr", label: "Geb\xFChr", einheit: "\u20AC" }]
};
const leerEinrichtung = () => ({
  id: Date.now(),
  name: "",
  ...
  erstellt: tagNr(),
  tarife: {}
  // { liegegebuehr:"45", sterbeurkunde:"12", ... }
});
async function dbLadeAdressbuch() {
  try {
    const r = await window.storage.get("bh_adressbuch");
    return r ? JSON.parse(r.value) : [];
  } catch {
    return [];
  }
}
async function dbSpeichereAdressbuch(liste) {
  try {
    await window.storage.set("bh_adressbuch", JSON.stringify(liste));
  } catch {
  }
}
const ZUGANGSCODES = {
  "hoffmann": { name: "Bestattungshaus Hoffmann", code: "HOFF2024", plan: "pro" },
  "conde": { name: "Bestattungen Cond\xE9", code: "COND2024", plan: "basic" },
  "krein": { name: "Bestattungen Krein", code: "KREI2024", plan: "basic" },
  "demo": { name: "Demo-Zugang", code: "DEMO1234", plan: "basic" }
};
const PLAN_LIMITS = {
  basic: { maxMitarbeiter: 999, maxFaelle: 999, label: "Basic", farbe: "#5B9EC9" },
  pro: { maxMitarbeiter: 999, maxFaelle: 999, label: "Pro", farbe: "#C5A55A" }
};
function pruefZugang(code) {
  const c = code.trim().toUpperCase();
  const z = Object.values(ZUGANGSCODES).find((z2) => z2.code === c);
  if (!z) return null;
  return { ...z, limits: PLAN_LIMITS[z.plan] || PLAN_LIMITS.basic };
}
function LoginScreen({ onLogin }) {
  ...
  const login = () => {
    if (!code.trim()) {
      setFehler("Bitte Zugangscode eingeben.");
      return;
    }
    setLaden(true);
    setTimeout(() => {
      const zugang = pruefZugang(code);
      if (zugang) {
        localStorage.setItem("bh_zugang", JSON.stringify({ code: code.trim().toUpperCase(), name: zugang.name, ts: Date.now() }));
        onLogin(zugang);
      } else {
        setFehler("Ung\xFCltiger Zugangscode. Bitte kontaktieren Sie uns.");
        setLaden(false);
      }
    }, 800);
  };
  return React.createElement(
    "div",
    {
      style: { minHeight: "100vh", background: "#13110E", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "24px", fontFamily: "Georgia,serif" }
    },
    React.createElement(
      "div",
      ...
      React.createElement(
        ...
        React.createElement("div", { style: { color: "#C5A55A", fontSize: 22, fontWeight: 700, marginBottom: 4 } }, "Bestattungshaus"),
        React.createElement("div", { style: { color: "#C5A55A", fontSize: 16 } }, "Team App"),
        React.createElement("div", { style: { color: "#444", fontSize: 13, marginTop: 8 } }, "Interne Verwaltungssoftware")
      ),
      // Login Card
      React.createElement(
        "div",
        { style: { background: "#1E1C19", borderRadius: 20, padding: "28px 24px", border: "1px solid rgba(197,165,90,.2)" } },
        React.createElement("div", { style: { color: "#F0EAE0", fontSize: 18, fontWeight: 700, marginBottom: 6 } }, "Anmelden"),
        React.createElement("div", { style: { color: "#666", fontSize: 14, marginBottom: 24 } }, "Bitte geben Sie Ihren Zugangscode ein."),
        React.createElement(
          "div",
          { style: { marginBottom: 16 } },
          React.createElement("div", { style: { color: "#AAA", fontSize: 12, marginBottom: 8 } }, "ZUGANGSCODE"),
          React.createElement("input", {
            ...
            style: {
              ...
              background: "#2A2825",
              border: `1px solid ${fehler ? "rgba(255,80,80,.5)" : "rgba(255,255,255,.15)"}`,
              borderRadius: 12,
              padding: "14px 16px",
              color: "#F0EAE0",
              fontSize: 20,
        ...
        fehler && React.createElement("div", { style: { color: "#ff6b6b", fontSize: 13, marginBottom: 16, padding: "10px 14px", background: "rgba(255,80,80,.1)", borderRadius: 10, border: "1px solid rgba(255,80,80,.2)" } }, "\u26A0 " + fehler),
        React.createElement("button", {
          onClick: login,
          disabled: laden,
          style: {
            width: "100%",
            background: laden ? "#333" : "#C5A55A",
            border: "none",
            borderRadius: 12,
            padding: "15px",
            color: laden ? "#666" : "#1C1A17",
            fontSize: 16,
            fontWeight: 700,
        ...
        }, laden ? "Wird gepr\xFCft \u2026" : "Anmelden \u2192")
      ),
      // Footer
      React.createElement(
        "div",
        { style: { textAlign: "center", marginTop: 24, color: "#333", fontSize: 12 } },
        "Kein Zugangscode? ",
        React.createElement("a", { href: "mailto:info@bestattungshaus-app.de", style: { color: "#C5A55A", textDecoration: "none" } }, "Kontakt aufnehmen")
      )
    )
  );
}
function MitarbeiterWahlScreen({ mitarbeiter, onWahl }) {
  return React.createElement(
    "div",
    { style: { minHeight: "100vh", background: "#13110E", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "24px", fontFamily: "Georgia,serif" } },
    React.createElement("div", { style: { width: "100%", maxWidth: 400 } },
      React.createElement("div", { style: { textAlign: "center", marginBottom: 36 } },
        ...
        React.createElement("div", { style: { color: "#C5A55A", fontSize: 22, fontWeight: 700, marginBottom: 4 } }, "Wer bist du?"),
        React.createElement("div", { style: { color: "#555", fontSize: 14 } }, "W\u00E4hle deinen Namen aus")
      ),
      React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 12 } },
        mitarbeiter.map((m) =>
          React.createElement("div", {
            key: m.id,
            onClick: () => onWahl(m),
            style: { display: "flex", alignItems: "center", gap: 16, background: "#1E1C19", border: "1px solid rgba(197,165,90,.2)", borderRadius: 16, padding: "16px 20px", cursor: "pointer", transition: "all .15s" }
          },
            React.createElement("div", { style: { width: 48, height: 48, borderRadius: "50%", background: m.farbe, color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, fontWeight: 800, flexShrink: 0 } }, m.kuerzel),
            React.createElement("div", null,
              React.createElement("div", { style: { color: "#F0EAE0", fontWeight: 700, fontSize: 17 } }, m.name),
              React.createElement("div", { style: { color: "#777", fontSize: 13 } }, m.rolle)
            ),
            React.createElement("div", { style: { marginLeft: "auto", color: "#C5A55A", fontSize: 22 } }, "\u203A")
          )
        )
      )
    )
...
async function dbLadeMitarbeiter() {
  try {
    const saved = localStorage.getItem("bh_mitarbeiter_v2");
    if (saved) return JSON.parse(saved);
    // Try Supabase as fallback
    const { data } = await sb.from("bh_mitarbeiter").select("data").limit(1);
    if (data?.[0]?.data) {
      localStorage.setItem("bh_mitarbeiter_v2", JSON.stringify(data[0].data));
      return data[0].data;
    }
    return INIT_MA;
  } catch {
...
async function dbSpeichereMitarbeiter(liste) {
  try {
    localStorage.setItem("bh_mitarbeiter_v2", JSON.stringify(liste));
    // Also try Supabase in background
    sb.from("bh_mitarbeiter").select("id").limit(1).then(({data}) => {
      if (data?.length) {
        sb.from("bh_mitarbeiter").update({ data: liste, updated_at: new Date().toISOString() }).eq("id", data[0].id);
      } else {
        sb.from("bh_mitarbeiter").insert({ data: liste });
      }
    }).catch(() => {});
  } catch (e) {
...
async function dbSpeichereFall(fall) {
  try {
    await sb.from("bh_faelle").upsert(
      { fall_id: String(fall.id), data: fall, updated_at: new Date().toISOString() },
      { onConflict: "fall_id" }
    );
  } catch (e) {
...
function Feld({ label, value, onChange, placeholder, typ = "text", warn }) {
  ...
  return /* @__PURE__ */ React.createElement("div", { style: { marginBottom: 14 } }, label && /* @__PURE__ */ React.createElement("div", { style: { color: "#AAA", fontSize: 12, marginBottom: 5, display: "flex", gap: 6, alignItems: "center" } }, label, warn && !value && /* @__PURE__ */ React.createElement("span", { style: { color: "#FF9500", fontSize: 11 } }, "\u26A0 fehlt")), /* @__PURE__ */ React.createElement(
    "input",
    {
      ref,
      type: typ,
...
function Textarea({ label, value, onChange, placeholder, warn, rows = 3 }) {
  const ref = useRef(null);
  useEffect(() => {
    if (ref.current && document.activeElement !== ref.current) {
      ref.current.value = value || "";
...
function MAWahlMulti({ value, onChange, mitarbeiter, label }) {
  // value is array of ids
  const vals = Array.isArray(value) ? value : (value ? [value] : []);
  const toggle = (id) => {
    if (vals.includes(id)) {
      onChange(vals.filter(x => x !== id));
    } else {
      onChange([...vals, id]);
    }
  };
  return /* @__PURE__ */ React.createElement("div", { style: { marginBottom: 14 } },
    /* @__PURE__ */ React.createElement("div", { style: { color: "#AAA", fontSize: 12, marginBottom: 8 } }, label),
    /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: 8 } },
      /* @__PURE__ */ React.createElement("div", { onClick: () => onChange([]), style: { padding: "8px 14px", borderRadius: 20, border: `2px solid ${vals.length === 0 ? "#C5A55A" : "rgba(255,255,255,.15)"}`, color: vals.length === 0 ? "#C5A55A" : "#888", cursor: "pointer", fontSize: 14, fontWeight: 600 } }, "Niemand"),
      mitarbeiter.map((m) => /* @__PURE__ */ React.createElement("div", { key: m.id, onClick: () => toggle(m.id), style: { display: "flex", alignItems: "center", gap: 8, padding: "8px 14px", borderRadius: 20, border: `2px solid ${vals.includes(m.id) ? m.farbe : "rgba(255,255,255,.15)"}`, background: vals.includes(m.id) ? `${m.farbe}25` : "transparent", cursor: "pointer", transition: "all .15s" } },
        /* @__PURE__ */ React.createElement("div", { style: { width: 24, height: 24, borderRadius: "50%", background: m.farbe, color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 9, fontWeight: 800 } }, m.kuerzel),
        /* @__PURE__ */ React.createElement("span", { style: { color: vals.includes(m.id) ? "#F0EAE0" : "#888", fontSize: 14, fontWeight: 600 } }, m.name.split(" ")[0])
      ))
    )
  );
}
...
function SchrittKarte({ schritt, mitarbeiter, onToggle, onNotiz, onVerantw }) {
  ...
  })(), offen && /* @__PURE__ */ React.createElement("div", { style: { padding: "0 16px 16px", borderTop: "1px solid rgba(255,255,255,.08)" } }, /* @__PURE__ */ React.createElement(MAWahlMulti, { value: schritt.verantw, onChange: onVerantw, mitarbeiter, label: "Zust\xE4ndig" }), /* @__PURE__ */ React.createElement("div", { style: { color: "#AAA", fontSize: 12, marginBottom: 6 } }, "Anmerkung"), /* @__PURE__ */ React.createElement(
    "textarea",
    {
      value: schritt.notiz,
...
function Drucken({ fall, mitarbeiter, onClose }) {
  const drucken = () => {
    ...
    if (!el) return;
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Einsargungsbericht</title><style>body{margin:0;padding:16mm 18mm;font-family:Georgia,serif;font-size:10.5pt;color:#1a1a1a}table{border-collapse:collapse}@page{size:A4;margin:0}</style></head><body>${el.innerHTML}</body></html>`;
    const blob = new Blob([html], { type: 'text/html' });
    const url = URL.createObjectURL(blob);
    // Try Web Share API first (iOS Safari)
    if (navigator.share) {
      const file = new File([blob], 'Einsargungsbericht.html', { type: 'text/html' });
      navigator.share({ title: 'Einsargungsbericht', files: [file] }).catch(() => {
...
function DokuFormular({ fall, d, mitarbeiter, updateDoku, adressbuch }) {
  const ei = d.eins || {};
...
function Dashboard({ faelle, mitarbeiter, ich, meineOffenen, setAktivId, setFallTab, setSeite, setNeuFallSheet, zugang }) {
  const CARD = { background: "#1E1C19", borderRadius: 16, padding: "16px" };
  return /* @__PURE__ */ React.createElement("div", { style: { padding: "16px 16px 100px" } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 12, marginBottom: 24, padding: "14px 16px", background: "#1E1C19", borderRadius: 16 } }, ich && /* @__PURE__ */ React.createElement(Avatar, { k: ich.kuerzel, farbe: ich.farbe, size: 48 }), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { color: "#F0EAE0", fontWeight: 700, fontSize: 17 } }, "Hallo, ", ich?.name.split(" ")[0]), /* @__PURE__ */ React.createElement("div", { style: { color: "#666", fontSize: 13 } }, ich?.rolle, " \xB7 ", TEAMS.find((t) => t.id === ich?.team)?.label)), meineOffenen.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { marginLeft: "auto", background: "#FF9500", color: "#fff", borderRadius: 12, padding: "4px 10px", fontSize: 13, fontWeight: 700 } }, meineOffenen.length, " offen")), zugang?.plan === "pro" && /* @__PURE__ */ React.createElement("div", { style: { display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 20 } }, [
    { label: "Aktive F\xE4lle", val: faelle.filter((f) => f.status !== "abgeschlossen").length, icon: "\u{1F4CB}", farbe: "#C5A55A" },
    { label: "Erledigt", val: faelle.filter((f) => f.status === "abgeschlossen").length, icon: "\u2705", farbe: "#4FA87A" },
    { label: "Mitarbeiter", val: mitarbeiter.length, icon: "\u{1F465}", farbe: "#5B9EC9" }
  ].map((s) => /* @__PURE__ */ React.createElement("div", { key: s.label, style: { background: "#1E1C19", borderRadius: 14, padding: "14px 10px", textAlign: "center", border: `1px solid ${s.farbe}30` } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 22, marginBottom: 4 } }, s.icon), /* @__PURE__ */ React.createElement("div", { style: { color: s.farbe, fontSize: 22, fontWeight: 700 } }, s.val), /* @__PURE__ */ React.createElement("div", { style: { color: "#666", fontSize: 11, marginTop: 2 } }, s.label)))), zugang?.plan !== "pro" && faelle.length >= (zugang?.limits?.maxFaelle || 50) - 5 && /* @__PURE__ */ React.createElement("div", { style: { background: "rgba(255,149,0,.08)", border: "1px solid rgba(255,149,0,.3)", borderRadius: 12, padding: "12px 16px", marginBottom: 16, display: "flex", alignItems: "center", gap: 12 } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 22 } }, "\u26A0\uFE0F"), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { color: "#FF9500", fontWeight: 700, fontSize: 14 } }, "Fast am Limit"), /* @__PURE__ */ React.createElement("div", { style: { color: "#888", fontSize: 13 } }, faelle.length, "/", zugang?.limits?.maxFaelle || 50, " F\xE4lle \u2014 Jetzt auf Pro upgraden f\xFCr unbegrenzte F\xE4lle."))), zugang?.plan === "pro" && faelle.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { marginBottom: 16 } }, /* @__PURE__ */ React.createElement("button", { onClick: () => {
    const rows = [["Fall", "Status", "Datum", "Ort", "Fortschritt"]];
    faelle.forEach((f) => {
      const abg = f.schritte.filter((s) => s.status === "abgeschlossen").length;
      rows.push([f.name, f.status, f.tod, f.ort || "", `${abg}/${f.schritte.length}`]);
    });
    const csv = rows.map((r) => r.map((c) => `"${String(c).replace(/"/g, '""')}"`).join(",")).join("\n");
    const blob = new Blob([csv], { type: "text/csv;charset=utf-8;" });
    const a = document.createElement("a");
    a.href = URL.createObjectURL(blob);
    a.download = `faelle_${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10)}.csv`;
    a.click();
  }, style: { width: "100%", background: "rgba(197,165,90,.1)", border: "1px solid rgba(197,165,90,.3)", borderRadius: 12, padding: "12px", color: "#C5A55A", cursor: "pointer", fontSize: 14, fontWeight: 700, fontFamily: "Georgia,serif", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 } }, "\u2B07 Alle F\xE4lle als CSV exportieren")), meineOffenen.length > 0 && /* @__PURE__ */ React.createElement("div", { style: { marginBottom: 24 } }, /* @__PURE__ */ React.createElement("div", { style: { color: "#888", fontSize: 12, letterSpacing: 1, marginBottom: 10 } }, "MEINE OFFENEN AUFGABEN"), meineOffenen.map((s, i) => /* @__PURE__ */ React.createElement("div", { key: i, onClick: () => {
    setAktivId(s.fallId);
    setFallTab("schritte");
    setSeite("fall");
...
function FallDetail({ fall, faelle, mitarbeiter, ichId, fallTab, setFallTab, notizText, setNotizText, updateFall, updateDoku, toggleSchritt, setDruckAnsicht, setLoeschFallId, addAktivitaet, adressbuch }) {
  const CARD = { background: "#1E1C19", borderRadius: 16, padding: "16px" };
...
function TeamMitarbeiterKarte({ m, mitarbeiter, setMA, ichId, setIchId, faelle }) {
  ...
  return /* @__PURE__ */ React.createElement("div", { style: { ...CARD, marginBottom: 10, display: "flex", alignItems: "center", gap: 14, border: "1px solid rgba(255,255,255,.08)" } }, /* @__PURE__ */ React.createElement(Avatar, { k: m.kuerzel, farbe: m.farbe, size: 48 }), /* @__PURE__ */ React.createElement("div", { style: { flex: 1, minWidth: 0 } }, /* @__PURE__ */ React.createElement("div", { style: { color: "#F0EAE0", fontWeight: 700, fontSize: 16 } }, m.name), /* @__PURE__ */ React.createElement("div", { style: { color: "#777", fontSize: 13 } }, m.rolle), m.email ? /* @__PURE__ */ React.createElement("div", { style: { color: "#5B9EC9", fontSize: 12, marginTop: 2 } }, "\u2709 ", m.email) : /* @__PURE__ */ React.createElement("div", { style: { color: "#555", fontSize: 12, marginTop: 2, fontStyle: "italic" } }, "Keine E-Mail hinterlegt"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 12, marginTop: 4 } }, /* @__PURE__ */ React.createElement("span", { style: { color: "#C5A55A", fontSize: 12 } }, faelleAnz, " F\xE4lle"), /* @__PURE__ */ React.createElement("span", { style: { color: "#5B9EC9", fontSize: 12 } }, aufg, " Aufgaben"))), editEmail ? /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexDirection: "column", gap: 6, minWidth: 160 } }, /* @__PURE__ */ React.createElement(
    ...
    setEmailVal(m.email || "");
    setEditEmail(true);
  }, style: { display: "none" } }, "\u2709"), bestaetige ? /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 4 } }, /* @__PURE__ */ React.createElement("button", { onClick: () => {
...
function Adressbuch({ adressbuch, setAdressbuch }) {
  ...
  const speichern = () => {
    ...
    if (editEntry) {
      setAdressbuch((p) => p.map((e) => e.id === editEntry ? { ...formData, id: editEntry } : e));
    } else {
      setAdressbuch((p) => [...p, { ...formData, id: Date.now() }]);
    }
    setNeuSheet(false);
    setEditEntry(null);
    setFormData(leerEinrichtung());
  };
  const loeschen = (id) => setAdressbuch((p) => p.filter((e) => e.id !== id));
  const katInfo = (id) => EINRICHTUNG_KATEGORIEN.find((k) => k.id === id) || EINRICHTUNG_KATEGORIEN.at(-1);
  const inputStyle = { width: "100%", boxSizing: "border-box", background: "#2A2825", border: "1px solid rgba(255,255,255,.15)", borderRadius: 10, padding: "12px 14px", color: "#F0EAE0", fontSize: 16, outline: "none", fontFamily: "Georgia,serif" };
  const CARD = { background: "#1E1C19", borderRadius: 16, padding: "16px" };
  return /* @__PURE__ */ React.createElement("div", { style: { padding: "16px 16px 100px" } }, /* @__PURE__ */ React.createElement("div", { style: { marginBottom: 16 } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 } }, /* @__PURE__ */ React.createElement("div", { style: { color: "#888", fontSize: 12, letterSpacing: 1 } }, "EINRICHTUNGEN (", adressbuch.length, ")"), /* @__PURE__ */ React.createElement(
    "button",
    {
      onClick: () => {
        setFormData(leerEinrichtung());
      ...
      style: { background: "#C5A55A", color: "#1C1A17", border: "none", borderRadius: 10, padding: "8px 16px", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "Georgia,serif" }
    },
    "\uFF0B Hinzuf\xFCgen"
  )), /* @__PURE__ */ React.createElement(
    "input",
    {
      ...
      placeholder: "\u{1F50D} Suchen \u2026",
      style: { ...inputStyle, marginBottom: 10 }
    }
  ), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", overflowX: "auto", gap: 8, paddingBottom: 4, WebkitOverflowScrolling: "touch" } }, /* @__PURE__ */ React.createElement("button", { onClick: () => setFilterKat("alle"), style: { background: filterKat === "alle" ? "#C5A55A" : "#2A2825", color: filterKat === "alle" ? "#1C1A17" : "#888", border: "none", borderRadius: 8, padding: "7px 14px", fontSize: 13, fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap", fontFamily: "Georgia,serif" } }, "Alle"), EINRICHTUNG_KATEGORIEN.map((k) => /* @__PURE__ */ React.createElement(
    "button",
    {
      key: k.id,
      onClick: () => setFilterKat(k.id),
      style: { background: filterKat === k.id ? `${k.farbe}33` : "#2A2825", color: filterKat === k.id ? k.farbe : "#888", border: `1px solid ${filterKat === k.id ? k.farbe : "transparent"}`, borderRadius: 8, padding: "7px 14px", fontSize: 13, fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap", fontFamily: "Georgia,serif" }
    },
    k.label
  )))), gefiltert.length === 0 && /* @__PURE__ */ React.createElement("div", { style: { ...CARD, textAlign: "center", padding: "40px 20px" } }, /* @__PURE__ */ React.createElement("div", { style: { fontSize: 40, marginBottom: 10 } }, "\u{1F4D2}"), /* @__PURE__ */ React.createElement("div", { style: { color: "#666", fontSize: 15, marginBottom: 16 } }, suche ? "Keine Treffer" : "Noch keine Einrichtungen erfasst")), gefiltert.map((e) => {
    const kat = katInfo(e.kategorie);
    return /* @__PURE__ */ React.createElement("div", { key: e.id, onClick: () => setDetail(e), style: { ...CARD, marginBottom: 10, cursor: "pointer", border: "1px solid rgba(255,255,255,.06)" } }, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "flex-start", gap: 12 } }, /* @__PURE__ */ React.createElement("div", { style: { width: 42, height: 42, borderRadius: 12, background: `${kat.farbe}22`, border: `1px solid ${kat.farbe}44`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, flexShrink: 0 } }, kat.label.split(" ")[0]), /* @__PURE__ */ React.createElement("div", { style: { flex: 1, minWidth: 0 } }, /* @__PURE__ */ React.createElement("div", { style: { color: "#F0EAE0", fontWeight: 700, fontSize: 16, marginBottom: 2 } }, e.name), /* @__PURE__ */ React.createElement("div", { style: { color: kat.farbe, fontSize: 12, fontWeight: 600, marginBottom: 4 } }, kat.label.split(" ").slice(1).join(" ")), e.adresse && /* @__PURE__ */ React.createElement("div", { style: { color: "#666", fontSize: 13, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" } }, "\u{1F4CD} ", e.adresse), e.telefon && /* @__PURE__ */ React.createElement("div", { style: { color: "#666", fontSize: 13 } }, "\u{1F4DE} ", e.telefon)), /* @__PURE__ */ React.createElement("div", { style: { color: "#444", fontSize: 18, flexShrink: 0 } }, "\u203A")));
  }), detail && /* @__PURE__ */ React.createElement(Sheet, { titel: detail.name, onClose: () => setDetail(null) }, (() => {
    const kat = katInfo(detail.kategorie);
    return /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 10, marginBottom: 20, padding: "12px", background: `${kat.farbe}15`, borderRadius: 12, border: `1px solid ${kat.farbe}30` } }, /* @__PURE__ */ React.createElement("span", { style: { fontSize: 28 } }, kat.label.split(" ")[0]), /* @__PURE__ */ React.createElement("span", { style: { color: kat.farbe, fontWeight: 700 } }, kat.label.split(" ").slice(1).join(" "))), [
      { icon: "\u{1F4DE}", label: "Telefon", val: detail.telefon, href: `tel:${detail.telefon}` },
      { icon: "\u2709", label: "E-Mail", val: detail.email, href: `mailto:${detail.email}` },
      { icon: "\u{1F4CD}", label: "Adresse", val: detail.adresse },
      { icon: "\u{1F464}", label: "Ansprechpartner", val: detail.ansprechpartner },
      { icon: "\u{1F550}", label: "\xD6ffnungszeiten", val: detail.oeffnungszeiten },
      { icon: "\u{1F4DD}", label: "Notizen", val: detail.notizen }
    ].filter((f) => f.val).map((f, i) => /* @__PURE__ */ React.createElement("div", { key: i, style: { marginBottom: 14, padding: "12px 14px", background: "#2A2825", borderRadius: 10 } }, /* @__PURE__ */ React.createElement("div", { style: { color: "#888", fontSize: 11, marginBottom: 4 } }, f.icon, " ", f.label), f.href ? /* @__PURE__ */ React.createElement("a", { href: f.href, style: { color: "#C5A55A", fontSize: 15, fontWeight: 600, textDecoration: "none" } }, f.val) : /* @__PURE__ */ React.createElement("div", { style: { color: "#F0EAE0", fontSize: 15 } }, f.val))), (TARIF_FELDER[detail.kategorie] || []).filter((tf) => (detail.tarife || {})[tf.key]).length > 0 && /* @__PURE__ */ React.createElement("div", { style: { background: "rgba(197,165,90,.06)", border: "1px solid rgba(197,165,90,.2)", borderRadius: 12, padding: "14px", marginBottom: 14 } }, /* @__PURE__ */ React.createElement("div", { style: { color: "#C5A55A", fontWeight: 700, fontSize: 14, marginBottom: 10 } }, "\u{1F4B0} Tarife & Geb\xFChren"), (TARIF_FELDER[detail.kategorie] || []).filter((tf) => (detail.tarife || {})[tf.key]).map((tf) => /* @__PURE__ */ React.createElement("div", { key: tf.key, style: { display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid rgba(255,255,255,.06)" } }, /* @__PURE__ */ React.createElement("span", { style: { color: "#888", fontSize: 14 } }, tf.label), /* @__PURE__ */ React.createElement("span", { style: { color: "#F0EAE0", fontWeight: 700, fontSize: 15 } }, (detail.tarife || {})[tf.key], " ", tf.einheit)))), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", gap: 10, marginTop: 8 } }, /* @__PURE__ */ React.createElement(
      "button",
      {
        onClick: () => {
          setFormData({ ...detail });
          setEditEntry(detail.id);
          setDetail(null);
          setNeuSheet(true);
        },
        style: { flex: 1, background: "rgba(197,165,90,.15)", border: "1px solid rgba(197,165,90,.3)", borderRadius: 10, padding: "12px", color: "#C5A55A", cursor: "pointer", fontSize: 15, fontWeight: 700, fontFamily: "Georgia,serif" }
      },
      "\u270F Bearbeiten"
    ), /* @__PURE__ */ React.createElement(
      "button",
      {
        ...
        style: { background: "rgba(255,80,80,.1)", border: "1px solid rgba(255,80,80,.2)", borderRadius: 10, padding: "12px 16px", color: "#ff6b6b", cursor: "pointer", fontSize: 18 }
      },
      "\u{1F5D1}"
    )));
  })()), neuSheet && /* @__PURE__ */ React.createElement(Sheet, { titel: editEntry ? "Einrichtung bearbeiten" : "Neue Einrichtung", onClose: () => {
    setNeuSheet(false);
    setEditEntry(null);
    setFormData(leerEinrichtung());
  } }, /* @__PURE__ */ React.createElement("div", { style: { marginBottom: 14 } }, /* @__PURE__ */ React.createElement("div", { style: { color: "#AAA", fontSize: 12, marginBottom: 8 } }, "KATEGORIE"), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", flexWrap: "wrap", gap: 8 } }, EINRICHTUNG_KATEGORIEN.map((k) => /* @__PURE__ */ React.createElement(
    "button",
    {
      key: k.id,
      onClick: () => setFormData((p) => ({ ...p, kategorie: k.id })),
      style: { background: formData.kategorie === k.id ? `${k.farbe}33` : "#2A2825", color: formData.kategorie === k.id ? k.farbe : "#888", border: `1px solid ${formData.kategorie === k.id ? k.farbe : "rgba(255,255,255,.1)"}`, borderRadius: 8, padding: "7px 12px", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "Georgia,serif" }
    },
    k.label
  )))), [
    { label: "Name *", key: "name", placeholder: "z. B. Klinikum K\xF6ln" },
    { label: "Adresse", key: "adresse", placeholder: "Stra\xDFe, PLZ Stadt" },
    { label: "Telefon", key: "telefon", placeholder: "+49 221 \u2026" },
    { label: "E-Mail", key: "email", placeholder: "info@beispiel.de" },
    { label: "Ansprechpartner", key: "ansprechpartner", placeholder: "Frau/Herr \u2026" },
    { label: "\xD6ffnungszeiten", key: "oeffnungszeiten", placeholder: "Mo-Fr 8-17 Uhr" },
    { label: "Notizen", key: "notizen", placeholder: "Besonderheiten, Zufahrt \u2026" }
  ].map((f) => /* @__PURE__ */ React.createElement("div", { key: f.key, style: { marginBottom: 14 } }, /* @__PURE__ */ React.createElement("div", { style: { color: "#AAA", fontSize: 12, marginBottom: 6 } }, f.label), f.key === "notizen" ? /* @__PURE__ */ React.createElement(
    "textarea",
    {
      ...
      rows: 3,
      style: { width: "100%", boxSizing: "border-box", background: "#2A2825", border: "1px solid rgba(255,255,255,.15)", borderRadius: 10, padding: "12px", color: "#F0EAE0", fontSize: 15, outline: "none", fontFamily: "Georgia,serif", resize: "none" }
    }
  ) : /* @__PURE__ */ React.createElement(
    "input",
    {
      ...
      onChange: (e) => setFormData((p) => ({ ...p, [f.key]: e.target.value })),
      placeholder: f.placeholder,
      style: { width: "100%", boxSizing: "border-box", background: "#2A2825", border: "1px solid rgba(255,255,255,.15)", borderRadius: 10, padding: "12px", color: "#F0EAE0", fontSize: 16, outline: "none", fontFamily: "Georgia,serif" }
    }
  ))), (TARIF_FELDER[formData.kategorie] || []).length > 0 && /* @__PURE__ */ React.createElement("div", { style: { background: "rgba(197,165,90,.06)", border: "1px solid rgba(197,165,90,.2)", borderRadius: 12, padding: "14px", marginBottom: 14 } }, /* @__PURE__ */ React.createElement("div", { style: { color: "#C5A55A", fontWeight: 700, fontSize: 14, marginBottom: 12 } }, "\u{1F4B0} Tarife & Geb\xFChren"), (TARIF_FELDER[formData.kategorie] || []).map((tf) => /* @__PURE__ */ React.createElement("div", { key: tf.key, style: { marginBottom: 10 } }, /* @__PURE__ */ React.createElement("div", { style: { color: "#AAA", fontSize: 12, marginBottom: 5 } }, tf.label), /* @__PURE__ */ React.createElement("div", { style: { display: "flex", alignItems: "center", gap: 8 } }, /* @__PURE__ */ React.createElement(
    "input",
    {
      value: (formData.tarife || {})[tf.key] || "",
      onChange: (e) => setFormData((p) => ({ ...p, tarife: { ...p.tarife || {}, [tf.key]: e.target.value } })),
      placeholder: "0.00",
      type: "number",
      min: "0",
      step: "0.01",
      style: { flex: 1, background: "#2A2825", border: "1px solid rgba(255,255,255,.15)", borderRadius: 8, padding: "10px 12px", color: "#F0EAE0", fontSize: 16, outline: "none", fontFamily: "Georgia,serif" }
    }
  ), /* @__PURE__ */ React.createElement("div", { style: { color: "#888", fontSize: 13, minWidth: 50 } }, tf.einheit))))), /* @__PURE__ */ React.createElement("button", { onClick: speichern, style: { width: "100%", background: formData.name.trim() ? "#4FA87A" : "#333", border: "none", borderRadius: 12, padding: "14px", color: formData.name.trim() ? "#fff" : "#666", cursor: formData.name.trim() ? "pointer" : "default", fontSize: 16, fontWeight: 700, fontFamily: "Georgia,serif" } }, editEntry ? "\u2713 Speichern" : "\u2713 Hinzuf\xFCgen")));
}
function App() {
  const [adressbuch, setAdressbuch] = useState([]);
  const [zugang, setZugang] = useState(() => {
    try {
      const z = localStorage.getItem("bh_zugang");
      if (!z) return null;
      const parsed = JSON.parse(z);
      if (Date.now() - parsed.ts > 30 * 24 * 60 * 60 * 1e3) {
        localStorage.removeItem("bh_zugang");
        return null;
      }
      return pruefZugang(parsed.code) ? parsed : null;
    } catch {
      return null;
    }
  });
  ...
  const [mitarbeiterGewaehlt, setMitarbeiterGewaehlt] = useState(() => {
    const saved = localStorage.getItem("bh_ich");
  ...
  useEffect(() => {
    (async () => {
      try {
        const [ma, faelleData, akts, adr] = await Promise.all([
          dbLadeMitarbeiter(),
          dbLadeFaelle(),
          dbLadeAktivitaeten(),
          dbLadeAdressbuch()
        ...
        setAdressbuch(adr);
        setMA(ma && ma.length > 0 ? ma : INIT_MA);
        ...
        const malist = ma && ma.length > 0 ? ma : INIT_MA;
        if (letzterUser && malist.find((m) => m.id === parseInt(letzterUser))) {
          setIchId(parseInt(letzterUser));
        } else if (malist.length > 0) {
          setIchId(malist[0].id);
        }
      } catch (e) {
        console.log("Lade-Fehler:", e);
        // On error, still show app with defaults
      }
      setGeladen(true);
    })();
  }, []);
  useEffect(() => {
    if (geladen) dbSpeichereAdressbuch(adressbuch);
  }, [adressbuch, geladen]);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      // Don't reload within 5 seconds of a save to avoid overwriting
      ...
      try {
        ...
        setAlleBenachrichtigungen(akts);
        setFaelle(prev => {
          const remoteIds = new Set(faelleData.map(f => f.id));
          const localOnly = prev.filter(f => !remoteIds.has(f.id));
          return [...localOnly, ...faelleData];
        });
  ...
  const addAktivitaet = (text, fallName, icon = "\u{1F4CC}") => {
    ...
    const _ts2 = eintrag.ts;
    setNeueBenachrichtigungen((p) => [eintrag, ...p].slice(0, 20));
    setTimeout(() => setNeueBenachrichtigungen((p) => p.filter((x) => x.ts !== _ts2)), 4000);
  };
  ...
  const updateFall = (id, upd) => setFaelle((p) => {
    ...
    if (geaendert) { lastSaveRef.current = Date.now(); dbSpeichereFall(geaendert); }
    return neu;
  });
  ...
  const toggleSchritt = (fallId, schrittId) => {
    ...
    updateFall(fallId, { schritte: f.schritte.map((s) => {
      ...
      const erledigtVon = n === "abgeschlossen" ? s.verantw || ichId : null;
      return { ...s, status: n, vonId: erledigtVon, amDatum: n === "abgeschlossen" ? `${tagNr()} ${uhrzeit()}` : null };
    }) });
    if (neuerStatus === "abgeschlossen") {
      const erledigtVon = schritt?.verantw || ichId;
      const name = mitarbeiter.find((m) => m.id === erledigtVon)?.name || "?";
      addAktivitaet(`Schritt erledigt: ${schritt?.titel} (${name})`, f.name, "\u2705");
  ...
  const fallAnlegen = () => {
    ...
    dbSpeichereFall(neu); // fire-and-forget
    addAktivitaet(`Neuer Fall angelegt: ${nf.name.trim()}`, nf.name.trim(), "\u{1F4CB}");
  };
  const maAnlegen = () => {
    if (!nma.name.trim()) return;
    const limit = zugang?.limits?.maxMitarbeiter || 8;
    if (mitarbeiter.length >= limit) {
      alert(`Ihr ${zugang?.limits?.label || "Basic"}-Plan erlaubt maximal ${limit} Mitarbeiter.

F\xFCr mehr Mitarbeiter bitte auf Pro upgraden.`);
      return;
    }
    const id = Date.now();
  ...
  if (!zugang) return React.createElement(LoginScreen, { onLogin: (z) => setZugang(z) });
  if (geladen && !mitarbeiterGewaehlt) return React.createElement(MitarbeiterWahlScreen, { mitarbeiter, onWahl: (m) => {
    setIchId(m.id);
    setMitarbeiterGewaehlt(m.id);
    localStorage.setItem("bh_ich", String(m.id));
  }});
