...
function App() {
  ...
  const addAktivitaet = (text, fallName, icon = "\u{1F4CC}") => {
    ...
    const _ts = eintrag.ts;
    setNeueBenachrichtigungen((p) => [eintrag, ...p].slice(0, 20));
    setTimeout(() => setNeueBenachrichtigungen((p) => p.filter((x) => x.ts !== _ts)), 4000);
  };
