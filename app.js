...
async function dbSpeichereMitarbeiter(liste) {
  try {
    // Use upsert with fixed row_key so there's always exactly one row
    await sb.from("bh_mitarbeiter").upsert(
      { row_key: "singleton", data: liste, updated_at: new Date().toISOString() },
      { onConflict: "row_key" }
    );
  } catch (e) {
    // Fallback: try plain insert if upsert not supported
    try {
    ...
    } catch(e2) { console.log("MA speichern:", e2); }
  }
}
